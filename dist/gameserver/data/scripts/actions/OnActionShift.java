...
public class OnActionShift extends Functions
{
	public boolean OnActionShift_NpcInstance(Player player, GameObject object)
	{
		...
		if(player.isGM())
		{
			...
			dialog = dialog.replace("%pevs%", String.valueOf(npc.getPEvasionRate(null)));
			dialog = dialog.replace("%pacc%", String.valueOf(npc.getPAccuracy()));
			dialog = dialog.replace("%mevs%", String.valueOf(npc.getMEvasionRate(null)));
			dialog = dialog.replace("%macc%", String.valueOf(npc.getMAccuracy()));
			dialog = dialog.replace("%pcrt%", String.valueOf(npc.getPCriticalHit(null)));
			dialog = dialog.replace("%mcrt%", String.valueOf(npc.getMCriticalHit(null, null)));
			...
			dialog = dialog.replace("%dist%", String.valueOf((int) npc.getDistance3D(player)));
	...
	public void droplist(String[] param)
	{
		...
		if(param.length == 0)
			...
			droplist(player, npc, RewardType.valueOf(param[0]), 0, 1);
		else if(param.length == 2)
			droplist(player, npc, RewardType.valueOf(param[0]), Integer.parseInt(param[1]), 1);
		else if(param.length > 2)
			droplist(player, npc, RewardType.valueOf(param[0]), Integer.parseInt(param[1]), Integer.parseInt(param[2]));
	}

	public void droplist(Player player, NpcInstance npc, RewardType showType, int showGroupId, int page)
	{
		if(player == null || npc == null)
			return;

		if(Config.ALT_GAME_SHOW_DROPLIST || player.isGM())
			RewardListInfo.showInfo(player, npc, showType, showGroupId, page);
	}
	...
	public void servitorEffects(String[] args)
	{
		...
		Player player = getSelf();
		Servitor servitor = player.getServitor(Integer.parseInt(args[0]));
		if(servitor == null)
			return;
	...
	public void stats()
	{
		...
		dialog = dialog.replace("%pAtk%", String.valueOf(npc.getPAtk(null)));
		dialog = dialog.replace("%mAtk%", String.valueOf(npc.getMAtk(null, null)));
		dialog = dialog.replace("%paccuracy%", String.valueOf(npc.getPAccuracy()));
		dialog = dialog.replace("%pevasionRate%", String.valueOf(npc.getPEvasionRate(null)));
		dialog = dialog.replace("%pcriticalHit%", String.valueOf(npc.getPCriticalHit(null)));
		dialog = dialog.replace("%maccuracy%", String.valueOf(npc.getMAccuracy()));
		dialog = dialog.replace("%mevasionRate%", String.valueOf(npc.getMEvasionRate(null)));
		dialog = dialog.replace("%mcriticalHit%", String.valueOf(npc.getMCriticalHit(null, null)));
		dialog = dialog.replace("%runSpeed%", String.valueOf(npc.getRunSpeed()));
		dialog = dialog.replace("%walkSpeed%", String.valueOf(npc.getWalkSpeed()));
	...
	public boolean OnActionShift_PetInstance(Player player, GameObject object)
	{
		...
		if(object.isPet())
		{
			...
			dialog = dialog.replace("%paccuracy%", String.valueOf(pet.getPAccuracy()));
			dialog = dialog.replace("%pevasionRate%", String.valueOf(pet.getPEvasionRate(null)));
			dialog = dialog.replace("%pcrt%", String.valueOf(pet.getPCriticalHit(null)));
			dialog = dialog.replace("%maccuracy%", String.valueOf(pet.getMAccuracy()));
			dialog = dialog.replace("%mevasionRate%", String.valueOf(pet.getMEvasionRate(null)));
			dialog = dialog.replace("%mcrt%", String.valueOf(pet.getMCriticalHit(null, null)));
			...
			dialog = dialog.replace("%dist%", String.valueOf((int) pet.getRealDistance(player)));
	...
	public boolean OnActionShift_ItemInstance(Player player, GameObject object)
	{
		...
		if(object.isItem())
		{
			...
			dialog = dialog.replace("%grade%", String.valueOf(item.getGrade()));
