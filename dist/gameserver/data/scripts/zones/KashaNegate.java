...
public class KashaNegate implements ScriptFile
{
		...
		public void onZoneLeave(Zone zone, Creature cha)
		{
			if(cha.isPlayable())
				for(int skillId : _buffs)
					cha.getEffectList().stopEffects(skillId);
		}
	}
					...
					if(yearningLvl > 0 || curseLvl > 0 || despairLvl > 0)
						for(Creature cha : zone.getInsidePlayables())
						{
							...
							if(curseLvl > 0)
							{
								...
								cha.getEffectList().stopEffects(_buffs[0]);
							if(yearningLvl > 0)
							{
								...
								cha.getEffectList().stopEffects(_buffs[1]);
							if(despairLvl > 0)
							{
								...
								cha.getEffectList().stopEffects(_buffs[2]);
	...
	private void addEffect(NpcInstance actor, Creature player, Skill skill, boolean animation)
	{
		if(skill.getLevel() > 0)
		{
			player.getEffectList().stopEffects(skill);
