	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		if(ROOM_ID == 2 || ROOM_ID == 3)
			if(Rnd.chance(5))
			{
				...
				skill.getEffects(getActor(), getActor());
			}
		if(ROOM_ID == 2)
			if(IS_LAST_GROUP && getActor().getCurrentHpPercents() < 80 && !getActor().getEffectList().containsEffects(ULTIMATE_BUFF_ID))
			{
				Skill skill = SkillHolder.getInstance().getSkill(ULTIMATE_BUFF_ID, 1);
				skill.getEffects(getActor(), getActor());
			}
	}
