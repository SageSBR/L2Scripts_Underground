...
public class Baylor extends DefaultAI
{
	...
	protected void onEvtSeeSpell(Skill skill, Creature caster)
	{
		...
		{
			_claw_count = 0;
			actor.getEffectList().stopEffects(Invincible);
	...
	protected boolean createNewTask()
	{
		...
		if(rnd_per < 7 && !actor.getEffectList().containsEffects(Berserk))
		{
		...
		if(rnd_per < 15 || rnd_per < 33 && !actor.getEffectList().containsEffects(Berserk))
		{
			return chooseTaskAndTargets(StrongPunch, target, distance);
		}

		//if(rnd_per < 5 && target.getEffectList().containsEffects(Imprison) == null)
		//{
		//	_isUsedInvincible = true;
