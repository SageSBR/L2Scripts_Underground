...
public class FreyaStandNormal extends Fighter
{
	...
	protected void thinkAttack()
	{
        ...
        Creature mostHated = actor.getAggroList().getMostHated(getMaxPursueRange());
		...
		{
			for(Effect e : actor.getEffectList().getEffects())
				if(e != null && e.isOffensive() && !actor.isSpecialEffect(e.getSkill()))
					e.exit();
		...
		if(System.currentTimeMillis() - _lastFactionNotifyTime > _minFactionNotifyInterval)
		{
			...
			for(NpcInstance npc : actor.getReflection().getNpcs())
				if(npc.isMonster() && npc != actor)
					npc.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, actor.getAggroList().getMostHated(getMaxPursueRange()), 5);
		}
		super.thinkAttack();
	}
