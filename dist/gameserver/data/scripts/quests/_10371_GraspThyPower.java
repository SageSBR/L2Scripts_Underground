...
public class _10371_GraspThyPower extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == gerkenshtein)
		{
			if(st.isCompleted())
				...
				if(checkStartCondition(st.getPlayer()) && ArrayUtils.contains(classesav, classid))
					htmltext = "start.htm";
				else
					return htmltext;
			} else if(cond == 1)
	...
	{
		QuestState qs = player.getQuestState(_10370_MenacingTimes.class);
		return player.getLevel() >= 76 && player.getLevel() <= 81 && qs != null && qs.getState() == COMPLETED;
	}	
}