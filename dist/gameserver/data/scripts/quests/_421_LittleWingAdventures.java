	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Mimyu)
		{
			...
			if(cond == 3)
			{
				...
				if(HatchlingSummoned(st, false) || st.getPlayer().getServitors().length > 0)
	...
	public class GuardiansSpawner extends RunnableImpl
	{
		...
		public GuardiansSpawner(NpcInstance npc, QuestState st, int _count)
		{
			...
			for(int i = 0; i < _count; i++)
			{
				...
				Servitor[] servitors = st.getPlayer().getServitors();
				agressors_pets = new ArrayList<String>(servitors.length);
		...
		private void updateAgression()
		{
			...
			if(_player != null)
			{
				...
				{
					Servitor[] servitors = _player.getServitors();
					if(servitors.length > 0)
					{
