...
public class _032_AnObviousLie extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getQuestItemsCount(PART_ARM) < 150 || st.getQuestItemsCount(GEM_FOR_ACC) < 100)
				htmltext = "You don't have enough materials";
		}
		else if(event.equalsIgnoreCase("cat") || event.equalsIgnoreCase("racoon") || event.equalsIgnoreCase("rabbit"))
			if(st.getCond() == 8 && st.getQuestItemsCount(PART_ARM) >= 150 && st.getQuestItemsCount(GEM_FOR_ACC) >= 100)
			{
				st.takeItems(PART_ARM, 150);
				st.takeItems(GEM_FOR_ACC, 100);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == GENTLER)
			...
			else if(cond == 8 && (st.getQuestItemsCount(PART_ARM) < 150 || st.getQuestItemsCount(GEM_FOR_ACC) < 100))
