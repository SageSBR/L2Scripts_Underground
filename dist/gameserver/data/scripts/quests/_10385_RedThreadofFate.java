...
public class _10385_RedThreadofFate extends Quest implements ScriptFile, OnMagicUseListener 
{
    ...
    public String onTalk(NpcInstance npc, QuestState st) 
	{
		...
		if(!st.getPlayer().getClassId().isOfLevel(ClassLevel.AWAKED))
        //if(st.getPlayer().getVar("q10338") == null)
			return "<html><head><body>You didn't complete quest Seize Your Destiny...</body></html>";

        if(st.getPlayer().getClassId().isOfRace(Race.ERTHEIA))
            return "<html><head><body>Not for Ertheia race...</body></html>";
	...
	{
        Reflection r = player.getActiveReflection(INSTANCE_ID);
        if(r != null) 
		{
