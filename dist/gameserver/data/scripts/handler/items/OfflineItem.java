...
public class OfflineItem extends SimpleItemHandler
{
	...
	public boolean forceUseItem(Playable playable, ItemInstance item, boolean ctrl)
	{
		...
		{
			player.sendMessage(new CustomMessage("itemshandlers.OfflineItem.LowLevel", player).addNumber(Config.SERVICES_OFFLINE_TRADE_MIN_LEVEL).toString());
			return true;
		}

		if(!player.isInStoreMode())
		{
			player.sendMessage(new CustomMessage("itemshandlers.OfflineItem.IncorrectUse", player).toString());
			return true;
		}
		...
		switch(Config.SERVICES_OFFLINE_TRADE_ALLOW_ZONE)
		{
			case 1:
				...
				{
					player.sendMessage(new CustomMessage("trade.OfflineNoTradeZoneOnlyPeace", player).toString());
					return true;
				}
				break;
			case 2:
				...
				{
					player.sendMessage(new CustomMessage("trade.OfflineNoTradeZoneOnlyOffshore", player).toString());
					return true;
				}
				break;
		}
		...
		{
			player.sendMessage(new CustomMessage("trade.OfflineNoTradeZone", player).toString());
			return true;
		}
