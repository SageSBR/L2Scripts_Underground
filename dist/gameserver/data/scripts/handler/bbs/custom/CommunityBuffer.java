...
public class CommunityBuffer extends ScriptsCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if(bypass.startsWith("_bbscancel"))
		{
			...
			{
				if(!eff.isOffensive() && (eff.getSkill().isMusic() || eff.getSkill().isSelfDispellable()) && !eff.getSkill().hasEffect(EffectUseType.NORMAL, EffectType.Transformation) && !player.isSpecialEffect(eff.getSkill()))
					eff.exit();
			}
	...
	private static boolean takeItemsAndBuff(Playable player, List<Skill> buffs, boolean toPet)
	{
		...
		if(player.getLevel() > BBSConfig.BUFF_SERVICE_MAX_LEVEL_FOR_FREE_BUFF)
		{
			if(Functions.getItemCount(player, BBSConfig.BUFF_SERVICE_COST_ITEM_ID) < needCount)
				return false;
		}
		...
		if(!toPet)
		{
			...
			if(target != null)
			{
				...
				{
					if(player.getLevel() > BBSConfig.BUFF_SERVICE_MAX_LEVEL_FOR_FREE_BUFF)
						Functions.removeItem(player, BBSConfig.BUFF_SERVICE_COST_ITEM_ID, needCount);
				}
				catch(Exception e)
				{
					return false;
		...
		else if(var[1].equalsIgnoreCase("add"))
		{
			...
			if(!availableBuffs.isEmpty())
			{
				...
				for(int i = minSkillIndex; i < maxSkillIndex; i++)
				{
					...
					String buff_str = "<td FIXWIDTH=5>&nbsp;</td>";
					buff_str = "<td FIXWIDTH=35 valign=top><img src=\"" + _buff.getIcon() + "\" width=32 height=32><br></td>";
					buff_str += "<td FIXWIDTH=40>" + htmlButton(">", 22, 32, "editset", "add", pageIdx, _buff.getId(), name) + "</td>";
					if(player.isLangRus())
						buff_str += "<td fixwidth=125><font color=3399FF>" + _buff.getName(player) + "</font><br1><font color=LEVEL> Уровень " + _buff.getLevel() + "</font></td>";
					else
						buff_str += "<td fixwidth=125><font color=3399FF>" + _buff.getName(player) + "</font><br1><font color=LEVEL> Level " + _buff.getLevel() + "</font></td>";
	...
	private static String pageMain(Player player)
	{
		...
		if(player.isLangRus())
			result.append(htmlButton("Все Баффы", 200, 25, "list", "0_0"));
		else
			result.append(htmlButton("All Buffs", 200, 25, "list", "0_0"));
		...
		if(!sets.isEmpty())
		{
			...
			if(player.isLangRus())
				result.append("<tr><td align=center>Стандартные наборы</td></tr>");
			else
				result.append("<tr><td align=center>Standart sets</td></tr>");
			...
			for(String setname : sets.keySet())
			{
				...
				result.append(htmlButton(name, 200, 25, "list", "1_0", setname));
				result.append("</td></tr>");
			}
		}
