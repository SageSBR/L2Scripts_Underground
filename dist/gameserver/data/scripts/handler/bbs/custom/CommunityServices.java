	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbsservices".equals(cmd))
		{
			...
			else if("cabinet".equals(cmd2))
			{
				...
				{
					html = HtmCache.getInstance().getNotNull("scripts/handler/bbs/pages/cabinet.htm", player);
					...
					html = html.replace("<?rate_xp?>", doubleToString(player.getRateExp(false,true)));
					html = html.replace("<?rate_sp?>", doubleToString(player.getRateSp(false,true)));
			...
			else if("changename".equals(cmd2))
			{
				...
				if("player".equals(cmd3))
				{
					...
					else
					{
						...
						else if(feeItemCount > 0 && ItemFunctions.removeItem(player, feeItemId, feeItemCount, true) != feeItemCount)
						{
			...
			else if("color".equals(cmd2))
			{
				...
				if("name".equals(cmd3))
				{
					...
					else
					{
						...
						if(feeItemCount > 0 && ItemFunctions.removeItem(player, feeItemId, feeItemCount, true) != feeItemCount)
						{
							String noHaveItemBlock = tpls.get(4);
							noHaveItemBlock = noHaveItemBlock.replace("<?fee_item_count?>", Util.formatAdena(feeItemCount));
							noHaveItemBlock = noHaveItemBlock.replace("<?fee_item_name?>", HtmlUtils.htmlItemName(feeItemId));
