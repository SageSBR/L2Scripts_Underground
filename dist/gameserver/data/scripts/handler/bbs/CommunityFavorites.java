	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("bbsgetfav".equals(cmd))
		{
			...
			try
			{
				...
				String tpl = HtmCache.getInstance().getNotNull("scripts/handler/bbs/bbs_favoritetpl.htm", player);
				while(rset.next())
				{
			...
			String html = HtmCache.getInstance().getNotNull("scripts/handler/bbs/bbs_getfavorite.htm", player);
