...
public class RignosInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if(command.equalsIgnoreCase("endRace"))
		{
			...
			if(count >= 4)
			{
				...
				player.getEffectList().stopEffects(SKILL_EVENT_TIMER);
	...
	public void showChatWindow(Player player, int val, Object... replace)
	{
		if(ItemFunctions.getItemCount(player, RACE_STAMP) >= 4)
			showChatWindow(player, "default/race_start001a.htm");
		else if(player.getLevel() >= 78 && _raceTask == null)
			showChatWindow(player, "default/race_start001.htm");
		else
			showChatWindow(player, "default/race_start002.htm");
	}
}