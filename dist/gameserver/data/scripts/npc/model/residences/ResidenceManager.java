		...
		else if(actualCommand.equalsIgnoreCase("functions"))
		{
			...
			if(val.equalsIgnoreCase("tele"))
			{
				...
				for(TeleportLocation loc : locs)
				{
					...
					teleport_list.append("<a action=\"bypass -h npc_%objectId%_teleport_");
			...
			else if(val.equalsIgnoreCase("support"))
			{
				...
				for(Object[] buff : allBuffs)
				{
					...
					String[] skillName = s.getName(player).split("\\s*-\\s*"); // Заглушка. На оффе пишется без приставки (Холл Клана - )
					support_list.append(skillName.length == 2 ? skillName[1] : s.getName(player));
