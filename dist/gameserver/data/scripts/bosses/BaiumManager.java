...
public class BaiumManager extends Functions implements ScriptFile, OnDeathListener
{
	...
	private final static int FWB_FIXINTERVALOFBAIUM = 7 * 24 * 60 * 60000;
	private final static int FWB_RANDOMINTERVALOFBAIUM = 2 * 24 * 60 * 60000;
	...
	{
		Calendar respawnDay = Calendar.getInstance();
		Calendar currentTime = Calendar.getInstance();

		respawnDay.set(Calendar.DAY_OF_WEEK, 1);
		respawnDay.set(Calendar.HOUR_OF_DAY, 16);
		respawnDay.set(Calendar.MINUTE, 0);
		respawnDay.set(Calendar.SECOND, 0);

		if(respawnDay.compareTo(currentTime) < 0)
			respawnDay.add(Calendar.DATE, 1);

		return (int)(respawnDay.getTimeInMillis() - currentTime.getTimeInMillis());
	}
