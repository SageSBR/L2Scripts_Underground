	...
	protected void runImpl()
	{
		...
		try
		{
			...
			for(MultiSellIngredient ingridient : entry.getIngredients())
			{
				...
				else if(ingridientItemId == ItemTemplate.ITEM_ID_RP)
				{

					if(activeChar.getRP() < totalAmount)
					{
						//activeChar.sendPacket(SystemMsg.YOU_DONT_HAVE_ENOUGH_REPUTATION_TO_DO_THAT);
						activeChar.sendMessage("У вас недостаточно очков РБ.");
						return;
					}
					if(!ingridient.getMantainIngredient())
						items.add(new ItemData(ingridientItemId, totalAmount, null));
			...
			for(ItemData id : items)
			{
				...
				if(count > 0)
					...
					else if(id.getId() == ItemTemplate.ITEM_ID_RP)
					{
						activeChar.setRP(activeChar.getRP() - (int) count);
						activeChar.sendPacket(new SystemMessagePacket(SystemMsg.S2_S1_HAS_DISAPPEARED).addLong(count).addString("RP"));
			...
			for(MultiSellIngredient in : products)
			{
				if(in.getItemId() <= 0)
				{
					...
					else if(in.getItemId() == ItemTemplate.ITEM_ID_RP)
					{
						activeChar.setRP(activeChar.getRP() + (int) (in.getItemCount() * _amount));
					}
				}
