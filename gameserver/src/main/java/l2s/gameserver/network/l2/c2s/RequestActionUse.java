...
public class RequestActionUse extends L2GameClientPacket
{
	...
	public static enum Action
	{
		...
		ACTION67(67, 0, 0), // Steer. Allows you to control the Airship.
		ACTION68(68, 0, 0), // Cancel Control. Relinquishes control of the Airship.
		ACTION69(69, 0, 0), // Destination Map. Choose from pre-designated locations.
		ACTION70(70, 0, 0), // Exit Airship. Disembarks from the Airship.
		...
		ACTION53(53, 1, 0), // Передвинуть пета к цели
		...
		ACTION54(54, 2, 0), // Передвинуть пета к цели
	...
	protected void runImpl()
	{
		...
		if(action.type == 0) // Действия с игроками
		{
			...
			switch(action.id)
			{
				...
				case 38: // Mount
				{
					...
					{
						activeChar.getEffectList().stopEffects(Skill.SKILL_EVENT_TIMER);
		...
		else if(action.type == 2) // Действия саммонов
		{
			...
			final GameObject target = activeChar.getTarget();
			final SummonInstance[] summons = activeChar.getSummons();
			if(summons.length < 1)
			{
				activeChar.sendActionFailed();
				return;
			...
			final SummonInstance summon = summons[0];
