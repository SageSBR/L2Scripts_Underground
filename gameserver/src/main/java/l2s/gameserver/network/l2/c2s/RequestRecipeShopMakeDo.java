...
public class RequestRecipeShopMakeDo extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(buyer.isGM())
			buyer.sendMessage("manufId: "+_manufacturerId+", recId: "+_price+", price: "+_price);

		if(buyer.isActionsDisabled())
		{
			if(buyer.isGM())
				buyer.sendMessage("чтото не так 1");
		...
		{
			if(buyer.isGM())
				buyer.sendMessage("чтото не так 2");
			buyer.sendActionFailed();
			return;
		}
		...
		{
			if(buyer.isGM())
				buyer.sendMessage("чтото не так 3");
			buyer.sendActionFailed();
			return;
		}
		...
		{
			if(buyer.isGM())
				buyer.sendMessage("чтото не так 4");
			buyer.sendActionFailed();
			return;
		}
		...
		{
			if(buyer.isGM())
				buyer.sendMessage("чтото не так 5");
			buyer.sendActionFailed();
			return;
		}
		...
		try
		{
			...
			manufacturer.addAdena(_price - tax);
		}
		finally
		{
		...
		if(Formulas.tryLuck(buyer) || Rnd.chance(recipe.getSuccessRate()))
		{
