...
public class RequestMagicSkillUse extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(isComboSkill(_magicId))
			skill = SkillHolder.getInstance().getSkill(_magicId, 1);
		else	
			skill = SkillHolder.getInstance().getSkill(_magicId, activeChar.getSkillLevel(_magicId));
		...
		if(skill != null)
		{
			...
			if(activeChar.getPendingLfcEnd() && skill.isOffensive())
			{
			...
			if(activeChar.isTransformed() && !activeChar.getAllSkills().contains(skill) && skill.getSkillType() != Skill.SkillType.CHANGE_CLASS)
			{
				activeChar.sendActionFailed();
				return;
			}
