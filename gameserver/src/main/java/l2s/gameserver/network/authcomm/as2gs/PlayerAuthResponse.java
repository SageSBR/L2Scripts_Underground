	...
	protected void runImpl()
	{
		...
		if(authed && client.getSessionKey().equals(skey))
		{
			if(Config.MAX_ACTIVE_ACCOUNTS_ON_ONE_IP > 0)
			{
				...
				if(!ignored && AuthServerCommunication.getInstance().getAuthedClientsByIP(client.getIpAddr()).size() >= Config.MAX_ACTIVE_ACCOUNTS_ON_ONE_IP)
				{
					client.close(new LoginFailPacket(LoginFailPacket.ACCESS_FAILED_TRY_LATER));
					return;
				}
			}
