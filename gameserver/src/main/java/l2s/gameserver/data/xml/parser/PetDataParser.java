 ...
 * @author Bonux
 */
public final class PetDataParser extends AbstractDirParser<PetDataHolder>
{
			...
			for(Iterator<Element> secondIterator = element.elementIterator(); secondIterator.hasNext();)
			{
				...
				if("expiration_reward_items".equalsIgnoreCase(secondElement.getName()))
				{
					for(Iterator<Element> thirdIterator = secondElement.elementIterator(); thirdIterator.hasNext();)
					{
						...
						if("item".equalsIgnoreCase(thirdElement.getName()))
						{
							...
							template.addExpirationRewardItem(new CapsuledItem(itemId, minCount, maxCount, chance));
						}
					}
				}
				else if("level_data".equalsIgnoreCase(secondElement.getName()))
				{
					for(Iterator<Element> thirdIterator = secondElement.elementIterator(); thirdIterator.hasNext();)
					{
						...
						if("stats".equalsIgnoreCase(thirdElement.getName()))
						{
							...
							final PetLevelData lvlData = new PetLevelData(stats_set);
