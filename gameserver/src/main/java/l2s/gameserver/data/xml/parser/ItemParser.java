			...
			for(Iterator<org.dom4j.Element> subIterator = itemElement.elementIterator(); subIterator.hasNext();)
			{
				...
				else if(subName.equalsIgnoreCase("skills"))
				{
					for(Iterator<org.dom4j.Element> nextIterator = subElement.elementIterator(); nextIterator.hasNext();)
					{
						...
						if(skill != null)
						{
							if(!skill.altUse() && (template.getHandler() instanceof SkillsItemHandler))
							{
								if(haveNoAltSkill)
									warn("Item:" + set.getObject("item_id") + " already has a \"no-alt\" skill: ID[" + skill.getId() + "] LEVEL[" + skill.getLevel() + "] that can lead to malfunction of item!");
								else
									haveNoAltSkill = true;

								if(!skill.isHandler())
									warn("Item:" + set.getObject("item_id") + " have \"no-handler\" skill: ID[" + skill.getId() + "] LEVEL[" + skill.getLevel() + "]!");
							}
				...
				else if(subName.equalsIgnoreCase("capsuled_items"))
				{
					for(Iterator<org.dom4j.Element> nextIterator = subElement.elementIterator(); nextIterator.hasNext();)
					{
						...
						if(nextElement.getName().equalsIgnoreCase("capsuled_item"))
						{
							...
							template.addCapsuledItem(new CapsuledItem(c_item_id, c_min_count, c_max_count, c_chance));
						}
					}
				}
