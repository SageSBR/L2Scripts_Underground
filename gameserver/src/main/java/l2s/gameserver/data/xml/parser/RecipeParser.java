...
public final class RecipeParser extends AbstractFileParser<RecipeHolder>
{
	...
	protected void readData(Element rootElement) throws Exception
	{
		for(Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();)
		{
			...
			final boolean awakedOnly = element.attributeValue("awaked_only") == null ? false : Boolean.parseBoolean(element.attributeValue("awaked_only"));

			RecipeTemplate recipe = new RecipeTemplate(id, level, mpConsume, successRate, itemId, isCommon, awakedOnly);
					...
					for(Element e : subElement.elements())
					{
						if("item".equalsIgnoreCase(e.getName()))
						{
							...
							if(Config.ALT_EASY_RECIPES)
							{
								...
								if(book_id != 0)
									recipe.addMaterial(new RecipeComponent(item_id, 1));
							}		
