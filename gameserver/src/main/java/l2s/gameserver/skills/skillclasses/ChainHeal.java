...
public class ChainHeal extends Skill
{
	...
	private boolean checkMainTarget(Creature activeChar, Creature target)
	{
		...
		if(target.isPlayer())
		{
			...
			if((activeCharTarget.isInDuel() && activeChar.getObjectId() != activeChar.getObjectId()) || activeCharTarget.isCursedWeaponEquipped() || (activeCharPlayer != null && !isTargetFriendly(activeCharPlayer, activeCharTarget)))
				return false;
		}
		return true;	
	}
	...
	public List<Creature> getTargets(Creature activeChar, Creature aimingTarget, boolean forceUse)
	{
		...
		if (targets != null && !targets.isEmpty() && ((activeCharPlayer != null && !activeCharPlayer.isInDuel()) || activeCharPlayer == null))
		{
			for(Creature target : targets)
			{
				...
				//  - Is dead.
				if(target == null ||
					((activeChar.getObjectId() != aimingTarget.getObjectId()) && (target.getObjectId() == activeChar.getObjectId())) ||
					target.isDoor() ||
					target.isMonster() ||
					activeChar.isAutoAttackable(target) ||
					target.isAlikeDead())
