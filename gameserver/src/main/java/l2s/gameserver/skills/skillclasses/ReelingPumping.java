...
public class ReelingPumping extends Skill
{
	...
	public void onEndCast(Creature activeChar, List<Creature> targets)
	{
		...
		double shotModifier = ((100 + player.getChargedFishshotPower()) / 100.);
		int pen = 0;
		double gradebonus = 1 + weaponItem.getGrade().ordinal() * 0.1;
		int dmg = (int) (getPower() * gradebonus * shotModifier);
