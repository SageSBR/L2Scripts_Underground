...
public class Call extends Skill
{
	...
	public static IStaticPacket canSummonHere(Player activeChar)
	{
		...
		if(activeChar.isInZoneBattle() || activeChar.isInZone(Zone.ZoneType.SIEGE) || activeChar.isInZone(no_restart) || activeChar.isInZone(no_summon) || activeChar.isInBoat() || activeChar.getReflection() != ReflectionManager.DEFAULT || activeChar.isInFightClub())
	...
	public static IStaticPacket canBeSummoned(Creature target)
	{
		...
		if(target.getPlayer().isInFightClub())
			return new SystemMessagePacket(SystemMsg.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING);
