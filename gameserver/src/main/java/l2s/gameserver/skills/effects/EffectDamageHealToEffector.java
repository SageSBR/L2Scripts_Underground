...
public class EffectDamageHealToEffector extends EffectDamOverTime
{
	private class DamageListener implements OnCurrentHpDamageListener
	{
		...
		{
			final Servitor[] servitors = _healServitors ? getEffector().getServitors() : new Servitor[0];

			final double hp = (damage * _hpAbsorbPercent / 100) / (servitors.length + 1);
			final double mp = (damage * _mpAbsorbPercent / 100) / (servitors.length + 1);
