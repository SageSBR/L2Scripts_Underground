...
public class EffectsDAO
{
	...
	public void restoreEffects(Playable playable)
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				final List<EffectTemplate> effectTemplates = skill.getEffectTemplates(isSelf ? EffectUseType.SELF : EffectUseType.NORMAL);
				for(EffectTemplate et : effectTemplates)
				{
					Env env = new Env(playable, playable, skill);
					Effect effect = et.getEffect(env);
					if(effect == null || !effect.isSaveable() || effect.getTemplate().isInstant())
						continue;
	...
	public void insert(Playable playable)
	{
		...
		final Effect[] effects = playable.getEffectList().getFirstEffects();
		if(effects.length == 0)
			return;
