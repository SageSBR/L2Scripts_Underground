...
public class CommissionManager
{
	...
	public boolean addCommissionItem(CommissionItem item)
	{
		...
		try
		{
			if(containsCommissionItem(item.getCommissionId())) //На всякий случай.
				return false;
	...
	public IStaticPacket registerItem(Player player, int itemObjId, long itemPrice, long itemCount, int periodDays)
	{
		...
		try
		{
			...
			try
			{
				...
				if(ItemFunctions.removeItem(player, item, itemCount, false) < itemCount) // Еще раз проверяем на всякий случай.
				{
