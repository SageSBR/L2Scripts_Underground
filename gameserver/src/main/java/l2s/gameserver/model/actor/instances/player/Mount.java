...
public class Mount
{
	...
	public void onRide()
	{
		switch(getType())
		{
			case STRIDER:
				if(_rider.isNoble())
				{
					_rider.addSkill(SkillHolder.getInstance().getSkill(Skill.SKILL_STRIDER_ASSAULT, 1), false);
			...
			case WYVERN:
				...
				_rider.addSkill(SkillHolder.getInstance().getSkill(Skill.SKILL_WYVERN_BREATH, 1), false);
				_rider.sendSkillList();
				break;
		}
	...
	public void onUnride()
	{
		...
		_rider.getEffectList().stopEffects(Skill.SKILL_HINDER_STRIDER);
		_rider.broadcastUserInfo(true); // нужно послать пакет перед Ride для корректного снятия оружия с заточкой
		_rider.broadcastPacket(new RidePacket(_rider));
		_rider.broadcastUserInfo(true); // нужно послать пакет после Ride для корректного отображения скорости
