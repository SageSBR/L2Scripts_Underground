...
public class PetInstance extends Servitor
{
	...
	private class FeedTask extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			if(item != null && !item.getTemplate().testCondition(getPlayer(), item)) // Если пет перестал соотвествовать кондишонам призыва, отзываем (офлайк).
				unSummon(false);
		}
	}
	...
	private void tryFeed()
	{
		...
		for(int foodId : getFoodId())
		{
			...
			if(food != null)
			{
				...
				{
					getInventory().destroyItem(food, 1);
	...
	public boolean useItem(ItemInstance item, boolean ctrlPressed, boolean force)
	{
		...
		if(success)
		{
			...
			{
				// TODO пробуем фиксить еду петомца.
	...
	{
		for(CapsuledItem ci : getData().getExpirationRewardItems())
		{
			if(Rnd.chance(ci.getChance()))
			{
				...
				ItemFunctions.addItem(getPlayer(), ci.getItemId(), count, true);
			}
		}
	}
}