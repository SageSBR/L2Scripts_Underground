...
public class MonsterInstance extends NpcInstance {
    ...
    private boolean _blockReward = false;
    ...
    public void setChampion(int level) {
        if (level == 0) {
            // Временное решение. Когда в пакете NpcInfo найдем параметр Team, убрать AbnormalEffect'ы
    ...
    public void rollRewards(RewardType type, RewardList list, final Creature lastAttacker, Creature topDamager) {
        ...
        double mod = type == RewardType.SWEEP ? calcStat(Stats.SPOIL_RATE_MULTIPLIER, 1., activeChar, null) : calcStat(Stats.DROP_RATE_MULTIPLIER, 1., activeChar, null);
        ...
        switch (type) {
            ...
            default:
                ...
                if (getChampion() > 0 && Config.SPECIAL_ITEM_ID > 0 && Math.abs(getLevel() - activePlayer.getLevel()) < 9 && Rnd.chance(Config.SPECIAL_ITEM_DROP_CHANCE)) {
                    Functions.addItem(activePlayer, Config.SPECIAL_ITEM_ID, Config.SPECIAL_ITEM_COUNT * getChampion());
                }
                break;
        }
    }
