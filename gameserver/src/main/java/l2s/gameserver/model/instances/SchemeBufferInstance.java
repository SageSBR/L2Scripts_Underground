...
*/
	...
	public void onBypassFeedback(Player player, String command)
	{
        ...
        if (!FREE_BUFFS)
        {
        	...
        	{
        		NpcHtmlMessagePacket msgNoAdena = new NpcHtmlMessagePacket(player, this);
		...
		else if (eventParam0.equalsIgnoreCase("giveBuffs"))
        {
            ...
            if ((int)(System.currentTimeMillis()/1000) > player.getblockUntilTime())
            {
                ...
                Functions.removeItem(player, CONSUMABLE_ID, cost);
                if (TIME_OUT)
                {
		...
		else if (eventParam0.equalsIgnoreCase("castBuffSet"))
        {
        	if ((int)(System.currentTimeMillis()/1000) > player.getblockUntilTime())
            {
                ...
                Functions.removeItem(player, CONSUMABLE_ID, BUFF_SET_PRICE);
                if (TIME_OUT)
                {
		...
		else if (eventParam0.equalsIgnoreCase("heal"))
        {
        	if ((int)(System.currentTimeMillis()/1000) > player.getblockUntilTime()) {
				...
				Functions.removeItem(player, CONSUMABLE_ID, HEAL_PRICE);
				if (TIME_OUT)
				{
		...
		else if (eventParam0.equalsIgnoreCase("removeBuffs"))
        {
        	if ((int)(System.currentTimeMillis()/1000) > player.getblockUntilTime())
            {
				...
				Functions.removeItem(player, CONSUMABLE_ID, BUFF_REMOVE_PRICE);
				if (TIME_OUT)
				{
					addTimeout(player, SetupGaugePacket.Colors.BLUE, TIME_OUT_TIME / 2, 600);
				}
		...
		else if (eventParam0.equalsIgnoreCase("cast"))
        {
			...
			Functions.removeItem(player, CONSUMABLE_ID, SCHEME_BUFF_PRICE);
