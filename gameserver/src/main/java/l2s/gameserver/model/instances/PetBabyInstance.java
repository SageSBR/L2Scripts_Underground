...
public final class PetBabyInstance extends PetInstance
{
				...
				if(!Config.ALT_PET_HEAL_BATTLE_ONLY || owner.isInCombat())
				{
					...
					if(curHp < 90 && Rnd.chance((100 - curHp) / 3))
						if(curHp < 33) // экстренная ситуация, сильный хил
							skill = SkillHolder.getInstance().getSkill(improved ? BattleHeal : GreaterHealTrick, getHealLevel());
						else if(getNpcId() != PetDataHolder.IMPROVED_BABY_KOOKABURRA_ID)
							skill = SkillHolder.getInstance().getSkill(improved ? GreaterHeal : HealTrick, getHealLevel());
					...
					if(skill == null && getNpcId() == PetDataHolder.IMPROVED_BABY_KOOKABURRA_ID) // Речардж только у Kookaburra и в комбат моде
					{
						...
						if(curMp < 66 && Rnd.chance((100 - curMp) / 2))
							skill = SkillHolder.getInstance().getSkill(Recharge, getRechargeLevel());
					}
	...
	private boolean checkEffect(Effect ef, Skill skill)
	{
		...
		final EffectTemplate effectTemplate = skill.getEffectTemplates(EffectUseType.NORMAL).get(0);
		if(!EffectList.checkAbnormalType(ef.getTemplate(), effectTemplate)) // такого скилла нет
			return false;
		if(ef.getAbnormalLvl() < effectTemplate.getAbnormalLvl()) // старый слабее
			return false;
	...
	private static final Skill[][] COUGAR_BUFFS = {
			{ SkillHolder.getInstance().getSkill(Pet_Empower, 3), SkillHolder.getInstance().getSkill(Pet_Might, 3) },
			{
					SkillHolder.getInstance().getSkill(Pet_Empower, 3),
					SkillHolder.getInstance().getSkill(Pet_Might, 3),
					SkillHolder.getInstance().getSkill(Pet_Shield, 3),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6) },
			{
					SkillHolder.getInstance().getSkill(Pet_Empower, 3),
					SkillHolder.getInstance().getSkill(Pet_Might, 3),
					SkillHolder.getInstance().getSkill(Pet_Shield, 3),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6),
					SkillHolder.getInstance().getSkill(Pet_Acumen, 3),
					SkillHolder.getInstance().getSkill(Pet_Haste, 2) },
			{
					SkillHolder.getInstance().getSkill(Pet_Empower, 3),
					SkillHolder.getInstance().getSkill(Pet_Might, 3),
					SkillHolder.getInstance().getSkill(Pet_Shield, 3),
					...
					SkillHolder.getInstance().getSkill(Pet_Acumen, 3),
					SkillHolder.getInstance().getSkill(Pet_Haste, 2),
					SkillHolder.getInstance().getSkill(Pet_Vampiric_Rage, 4),
					SkillHolder.getInstance().getSkill(Pet_Focus, 3) } };

	private static final Skill[][] BUFFALO_BUFFS = {
			{ SkillHolder.getInstance().getSkill(Pet_Might, 3), SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6) },
			{
					SkillHolder.getInstance().getSkill(Pet_Might, 3),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6),
					SkillHolder.getInstance().getSkill(Pet_Shield, 3),
					SkillHolder.getInstance().getSkill(Pet_Guidance, 3), },
			{
					SkillHolder.getInstance().getSkill(Pet_Might, 3),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6),
					SkillHolder.getInstance().getSkill(Pet_Shield, 3),
					...
					SkillHolder.getInstance().getSkill(Pet_Haste, 2) },
			{
					...
					SkillHolder.getInstance().getSkill(Pet_Haste, 2),
					SkillHolder.getInstance().getSkill(Pet_Focus, 3),
					SkillHolder.getInstance().getSkill(Pet_Death_Wisper, 3) } };

	private static final Skill[][] KOOKABURRA_BUFFS = {
			{ SkillHolder.getInstance().getSkill(Pet_Empower, 3), SkillHolder.getInstance().getSkill(Pet_Blessed_Soul, 6) },
			{
					SkillHolder.getInstance().getSkill(Pet_Empower, 3),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Soul, 6),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6),
					SkillHolder.getInstance().getSkill(Pet_Shield, 3) },
			{
					SkillHolder.getInstance().getSkill(Pet_Empower, 3),
					...
					SkillHolder.getInstance().getSkill(Pet_Shield, 3),
					SkillHolder.getInstance().getSkill(Pet_Acumen, 3),
					SkillHolder.getInstance().getSkill(Pet_Concentration, 6) },
			{
					SkillHolder.getInstance().getSkill(Pet_Empower, 3),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Soul, 6),
					SkillHolder.getInstance().getSkill(Pet_Blessed_Body, 6),
