...
public class PcInventory extends Inventory
{
	...
	public ItemInstance findEquippedLure()
	{
		...
		for(ItemInstance temp : getItems())
		{
			if(temp.getItemType() == EtcItemType.LURE)
			{
				if(temp.getLocation() == ItemLocation.PAPERDOLL && temp.getEquipSlot() == PAPERDOLL_LHAND)
					return temp;
	...
	protected void onRestoreItem(ItemInstance item)
	{
		...
		if(!startVisualChangeTask(item) && item.isTemporalItem())
	...
	@Override
	public void sendEquipInfo(ItemInstance item)
	{
		getActor().broadcastUserInfo(true);
		getActor().sendPacket(new ExUserInfoEquipSlot(getActor()));
	}
	...
	public void stopAllTimers()
	{
		for(ItemInstance item : getItems())
		{
			if(item.isShadowItem() || item.isTemporalItem() || item.getVisualId() > 0 && item.getLifeTime() > 0)
				item.stopTimer();
		}
	}
