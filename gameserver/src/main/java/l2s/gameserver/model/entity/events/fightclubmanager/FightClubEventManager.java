...
public class FightClubEventManager
{
	...
	public enum CLASSES
	{
		FIGHTERS(13113, ClassId.DUELIST, ClassId.DREADNOUGHT, ClassId.TITAN, ClassId.GRAND_KHAVATARI, ClassId.MAESTRO, ClassId.DOOMBRINGER, ClassId.M_SOUL_HOUND, ClassId.F_SOUL_HOUND, ClassId.RANGER_GRAVITY),
		TANKS(13112, ClassId.PHOENIX_KNIGHT, ClassId.HELL_KNIGHT, ClassId.EVAS_TEMPLAR, ClassId.SHILLIEN_TEMPLAR, ClassId.TRICKSTER),
		ARCHERS(13114, ClassId.SAGITTARIUS, ClassId.MOONLIGHT_SENTINEL, ClassId.GHOST_SENTINEL, ClassId.FORTUNE_SEEKER),
		DAGGERS(13114, ClassId.ADVENTURER, ClassId.WIND_RIDER, ClassId.GHOST_HUNTER),
		MAGES(13116, ClassId.ARCHMAGE, ClassId.SOULTAKER, ClassId.MYSTIC_MUSE, ClassId.STORM_SCREAMER, ClassId.SAIHA_RULER),
		SUMMONERS(13118, ClassId.ARCANA_LORD, ClassId.ELEMENTAL_MASTER, ClassId.SPECTRAL_MASTER),
		HEALERS(13115, ClassId.CARDINAL, ClassId.EVAS_SAINT, ClassId.SHILLIEN_SAINT, ClassId.DOMINATOR),
		SUPPORTS(13117, ClassId.HIEROPHANT, ClassId.SWORD_MUSE, ClassId.SPECTRAL_DANCER, ClassId.DOOMCRYER, ClassId.JUDICATOR);
	...
	public void startEventCountdown(AbstractFightClub event)
	{
		...
		ThreadPoolManager.getInstance().schedule(new Runnable(){
			...
			public void run()
			{
				...
				try
				{
					Thread.sleep(30000); //IQMAN TEST 3 * 60000
				}
				...
				try
				{
					Thread.sleep(3000); //IQMAN TEST 45000
				}
				catch(InterruptedException e)
				{
					e.printStackTrace();
	...
	/**
	 * Checking {@link #canPlayerParticipate(l2s.gameserver.model.Player, boolean, boolean)} for every player participated in event. Sending Message, checking all condtions
	...
	public void requestEventPlayerMenuBypass(Player player, String bypass)
	{
		...
		switch(action)
		{
			...
			case "buffer":
				...
				if(handler != null)
					handler.onBypassCommand(player, "_bbsbuffer");
				break;
		}
	}
	...
	public void sendEventPlayerMenu(Player player)
	{
		...
		player.sendPacket(new TutorialShowHtmlPacket(1, builder.toString())); //normal wnd
	...
	private void leaveEvent(Player player)
	{
		...
		if(event == null)
		{
			player.teleToLocation(RETURN_LOC, ReflectionManager.DEFAULT);
			System.out.println("Player "+player.getName()+" request out of event but no event found -> dissmised");
			return;
		}

		if(event.leaveEvent(player, true, true))
