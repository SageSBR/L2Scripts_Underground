...
public class ChaosFestivalEvent extends GlobalEvent
{
	...
	private void startRegistration()
	{
		...
		{
			if(player.getBaseSubClass().getLevel() < 85 /*|| player.getClan() == null || player.getClan().getLevel() < 3*/)
				continue;
	...
	private boolean checkParticipationCond(Player player, boolean msg)
	{
		...
		{
			if(msg)
				player.sendMessage("Вы должны быть в клане 3+ лвл для участия в фестивале.");
			return false;
		}
		...
		{
			if(msg)
				player.sendMessage("Вы должны иметь как минимум 3 профессию для участия в фестивале.");
			return false;
		}
		...
		{
			if(msg)
				player.sendPacket(SystemMsg.ONLY_CHARACTERS_LEVEL_85_OR_ABOVE_MAY_PARTICIPATE_IN_THE_TOURNAMENT);
			return false;
		}
		...
		{
			if(msg)
				player.sendMessage("Запрещено участвовать в фестивале с проклятым оружием.");
			return false;
		}
		...
		for(ChaosFestivalPlayerObject registeredPlayer : registeredPlayers)
		{
			Player _player = registeredPlayer.getPlayer();
			if(_player == null || !_player.isOnline() || _player == player)
				continue;
			if(_player.getHWID().equalsIgnoreCase(player.getHWID()))
			{
				if(msg)
					player.sendMessage("Запрещено участвовать твинками!");
				return false;
			}
		}
