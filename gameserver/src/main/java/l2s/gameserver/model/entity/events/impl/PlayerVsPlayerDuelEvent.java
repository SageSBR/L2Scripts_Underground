...
public class PlayerVsPlayerDuelEvent extends DuelEvent
{
	private class DuelListeners implements OnAttackListener, OnMoveListener, OnTeleportedListener
	{
		...
		{
			if(actor.getEvent(DuelEvent.class) != target.getEvent(DuelEvent.class))
				stopEvent();
		}
		...
		public void onMove(Creature actor, Location loc)
		{
			...
			{
				if(d.getPlayer() != actor && !actor.isInRangeZ(d.getPlayer(), 1200))
					stopEvent();
			}
		}
		...
		public void onTeleported(Player player)
		{
			for(DuelSnapshotObject d : PlayerVsPlayerDuelEvent.this)
			{
				if(d.getPlayer() != player && !player.isInRangeZ(d.getPlayer(), 1200))
					playerExit(player);
			}
		}
	}
	...
	public void askDuel(Player player, GameObject target)
	{
		...
		if(target.isPlayer())
		{
			...
			target.getPlayer().sendPacket(new SystemMessagePacket(SystemMsg.C1_HAS_CHALLENGED_YOU_TO_A_DUEL).addName(player), new ExDuelAskStart(player.getName(), 0));
		}
	}
