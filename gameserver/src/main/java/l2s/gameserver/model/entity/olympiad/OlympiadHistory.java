...
public class OlympiadHistory
{
	...
	public String toString(Player player, int target, int wins, int loss, int tie)
	{
		...
		String main = null;
		if(_gameStatus == 0)
			main = StringsHolder.getInstance().getNotNull(player, "hero.history.tie");
		else if(team == _gameStatus)
			main = StringsHolder.getInstance().getNotNull(player, "hero.history.win");
		else
			main = StringsHolder.getInstance().getNotNull(player, "hero.history.loss");
