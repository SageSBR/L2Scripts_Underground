...
public final class ChaosFestivalArenaObject implements Serializable, Comparable<ChaosFestivalArenaObject>
{
	...
	private void calcWinner()
	{
		...
		if(winner.getKills() > 0)
			_winner = winner;
//poka tak TODO listner
		for(ChaosFestivalPlayerObject member : _members)
		{
			if(member.getPlayer() == null)
				continue;
			Quest q1 = QuestManager.getQuest(10748);	
			Quest q2 = QuestManager.getQuest(10749);
			String qn1 = q1.getName();
			String qn2 = q2.getName();
			QuestState qs1 = member.getPlayer().getQuestState(qn1);
			QuestState qs2 = member.getPlayer().getQuestState(qn2);
			if(qs1 == null && qs2 == null)
				break;
			if(qs1 != null)
				q1.onHaosBattleEnd(member.getPlayer(), false);
			else
				q2.onHaosBattleEnd(member.getPlayer(), false);
		}			
	}
