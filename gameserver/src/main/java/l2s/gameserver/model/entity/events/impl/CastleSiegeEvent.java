...
public class CastleSiegeEvent extends SiegeEvent<Castle, SiegeClanObject>
{
	...
	public void takeCastle(Clan newOwnerClan, ResidenceSide side)
	{
		...
		getResidence().setResidenceSide(side, true);
	...
	public void processStep(Clan newOwnerClan)
	{
		...
		for(CastleSiegeEvent castleSiege : EventHolder.getInstance().getEvents(CastleSiegeEvent.class))
		{
			...
			System.out.println("processStep remove from siege: " + castleSiege.getId() + " 1");
			...
			if(siegeClan != null)
			{
				...
				System.out.println("processStep remove from siege: " + castleSiege.getId() + " 2");
			}
			...
			if(siegeClan != null)
			{
				...
				System.out.println("processStep remove from siege: " + castleSiege.getId() + " 3");
			}
		}
	...
	public void generateNextSiegeDates()
	{
		...
		validateSiegeDate(calendar, _siegeIntervalInWeeks);
	...
	public boolean canRessurect(Player resurrectPlayer, Creature target, boolean force)
	{
		...
		List<CastleSiegeEvent> siegeEvents = targetPlayer.getEvents(CastleSiegeEvent.class);
		List<CastleSiegeEvent> siegeEvents2 = resurrectPlayer.getEvents(CastleSiegeEvent.class);

		if(siegeEvents.isEmpty() || siegeEvents2.isEmpty() || !siegeEvents.contains(this) || !siegeEvents2.contains(this))
		{
			if(force)
