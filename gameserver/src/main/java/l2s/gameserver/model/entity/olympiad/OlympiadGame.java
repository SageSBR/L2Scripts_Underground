...
public class OlympiadGame
{
	...
	private void winGame(OlympiadMember winnerMember, OlympiadMember looseMember)
	{
		...
		{
			ItemInstance item = winnerPlayer.getInventory().addItem(45584, 12);  /*Config.ALT_OLY_BATTLE_REWARD_ITEM, getType().getReward()*/
			winnerPlayer.sendPacket(SystemMessagePacket.obtainItems(item.getItemId(), getType().getReward(), 0));
			winnerPlayer.sendChanges();
		}

		Player loosePlayer = looseMember.getPlayer();
		if(loosePlayer != null)
		{
			ItemInstance item = loosePlayer.getInventory().addItem(45584, 7);
			loosePlayer.sendPacket(SystemMessagePacket.obtainItems(item.getItemId(), getType().getReward(), 0));
			loosePlayer.sendChanges();
		}
