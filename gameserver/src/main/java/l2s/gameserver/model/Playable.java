...
public abstract class Playable extends Creature
{
	...
	public boolean checkTarget(Creature target)
	{
		...
		if(target.isPlayable())
		{
			...
			if(target.getPlayer().isInFightClub() && !player.isInFightClub())
				return false; //if not in event not fighting
			if(!target.getPlayer().isInFightClub() && player.isInFightClub())
				return false; //if not in event not fighting	 opposite		
			if(target.getPlayer().isInFightClub() && player.isInFightClub())
			{
				if(target.getPlayer().getFightClubEvent().isFriend(target, player))
					return false; //can't attack friends on events
			}			
	...
	{
		if(!isPlayer()) // TODO: [Bonux] Проверить на оффе.
			return 1.;
	...
	public Collection<Entry<TimeStamp>> getSharedGroupReuses()
	{
	...
	public abstract Skill getAdditionalSSEffect(boolean spiritshot, boolean blessed);
}