...
import l2s.gameserver.network.l2.s2c.NpcInfoPacket.ExPetInfo;
...
public abstract class Servitor extends Playable
{
	...
	{
		Player owner = getPlayer();
		if(!isTargetable(player) && (!_targetable || player != owner))
		{
	...
	public void broadcastCharInfoImpl()
	{
		...
		for(Player player : World.getAroundPlayers(this))
		{
			...
			{
				if(isPet())
					player.sendPacket(new ExPetInfo((PetInstance) this, player).update());
				else if(isSummon())
					player.sendPacket(new SummonInfoPacket((SummonInstance) this, player).update());
				else
	...
	public List<L2GameServerPacket> addPacketList(Player forPlayer, Creature dropper)
	{
		...
		else if(!getPlayer().isInvisible())
		{
			...
			if(isPet())
				list.add(new ExPetInfo((PetInstance) this, forPlayer));
			else if(isSummon())
				list.add(new SummonInfoPacket((SummonInstance) this, forPlayer));
			else
	...
	public void transferOwnerBuffs()
	{
		...
		if(!effects.isEmpty())
		{
			for(Effect e : effects)
			{
				...
				if(effect == null || effect.getTemplate().isInstant())
					continue;
	...
	public Skill getAdditionalSSEffect(boolean spiritshot, boolean blessed)
	{
		...
		{
			// Ruby's
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38855, 0)) // Ruby - Lv. 1
				return SkillHolder.getInstance().getSkill(17888, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38856, 0)) // Ruby - Lv. 2
				return SkillHolder.getInstance().getSkill(17888, 2);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38857, 0)) // Ruby - Lv. 3
				return SkillHolder.getInstance().getSkill(17889, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38858, 0)) // Ruby - Lv. 4
				return SkillHolder.getInstance().getSkill(17890, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 38859, 0)) // Ruby - Lv. 5
				return SkillHolder.getInstance().getSkill(17891, 1);
		}
