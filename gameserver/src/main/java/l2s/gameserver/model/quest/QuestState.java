...
public final class QuestState
{
				...
				case Quest.COMMAND_CHANNEL:
					...
					else
					{
						...
						for(Player p : actorPlayer.getPlayer().getParty().getCommandChannel())
							if(p.isInRange(actorPlayer, Creature.INTERACTION_DISTANCE))
								players.add(p);
					}
					break;					
				default:
	...
	public Object setState(int state)
	{
		...
		if(getQuest().isVisible(player) && isStarted())
			player.sendPacket(new ExShowQuestMarkPacket(getQuest().getQuestIntId(), getCond()));
	...
	public boolean isSecondTimeAvailable(Player player)
	{
		SubClass sub = player.getActiveSubClass();
		if(sub == null || !sub.isDual())
			return false;
		return player.getVar(getQuest().getName()) != null;
	}
}