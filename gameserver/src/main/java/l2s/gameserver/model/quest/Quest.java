...
import l2s.gameserver.model.quest.startcondition.impl.PlayerCurrentRaceCondition;
...
public class Quest
{
	...
	public static final int COMMAND_CHANNEL = 3;
	...
	private TIntObjectHashMap<List<QuestNpcLogInfo>> _npcLogList = new TIntObjectHashMap<List<QuestNpcLogInfo>>(5);
	private TIntObjectHashMap<List<QuestNpcLogInfo>> _itemsLogList = new TIntObjectHashMap<List<QuestNpcLogInfo>>(5);
	private List<ICheckStartCondition> _startConditionList = new ArrayList<ICheckStartCondition>();
	...
	public static void restoreQuestStates(Player player)
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				{
					//_log.error("ADMINISTRATOR PAY ATTANTION: QUEST "+getClass().getSimpleName()+" MUST BE FIXED TRANSFER TO IQMAN"); //debug
				}
	...
	public Quest(int party)
	{
		...
		if(!Config.EX_USE_QUEST_REWARD_PENALTY_PER || !Config.EX_F2P_QUEST_REWARD_PENALTY_QUESTS.contains(_questId))
			_rewardRate = 1.;
		else
	...
	public void addRaceCheck(boolean human, boolean elf, boolean delf, boolean orc, boolean dwarf, boolean kamael, boolean ertheia)
	{
		_startConditionList.add(new PlayerRaceCondition(human, elf, delf, orc, dwarf, kamael, ertheia));
	}

	public void addCurrentRaceCheck(boolean human, boolean elf, boolean delf, boolean orc, boolean dwarf, boolean kamael, boolean ertheia)
	{
		_startConditionList.add(new PlayerCurrentRaceCondition(human, elf, delf, orc, dwarf, kamael, ertheia));
	}
	/**
	 ...
	 */
	public void addClassLevelCheck(boolean ertheia, ClassLevel... classLevels)
	{
		_startConditionList.add(new ClassLevelCondition(ertheia, classLevels));
	}
	...
	public void addClassIdCheck(int... classIds)
	{
		_startConditionList.add(new ClassIdCondition(classIds));
	}

	public void addClassIdCheck(ClassId... classIds)
	{
		_startConditionList.add(new ClassIdCondition(classIds));
	}
	...
	public final int getDescrState(Player player, boolean isStartNpc)
	{
		...
		int state = 4;
		if(checkStartCondition(player) && isStartNpc && (qs == null || qs.isCreated() && qs.isNowAvailable()))
			state = 1;
		else if(qs != null && qs.isStarted()) // TODO [Ragnarok], уточнить нужна ли здесь проверка на checkStartCondition()
			state = 2;
		else if(qs != null && (qs.isCompleted() || !qs.isNowAvailable()))
			state = 3;
		return state;
	}
	...
	protected void showHtmlFile(Player player, NpcInstance npc, String fileName, boolean showQuestInfo, Object... arg)
	{
		...
		NpcHtmlMessagePacket npcReply = showQuestInfo ? new ExNpcQuestHtmlMessage(player, npc, getQuestIntId()) : new NpcHtmlMessagePacket(player, npc);
	...
	{
		NpcInstance result = Functions.spawn(randomOffset > 50 ? Location.findPointToStay(loc, 50, randomOffset, ReflectionManager.DEFAULT.getGeoIndex()) : loc, npcId);
	...
	protected final void enterInstance(QuestState st, Reflection reflection, int instancedZoneId, Object... args)
	{
		Player player = st.getPlayer();
		if(player == null)
			return;
	...
	protected final void enterInstance(QuestState st, int instancedZoneId, Object... args)
	{
		enterInstance(st, new Reflection(), instancedZoneId, args);
	}
