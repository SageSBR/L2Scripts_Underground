...
public class Fishing
{
	...
	public void stopFishing()
	{
		...
		_fisher.broadcastPacket(new ExUserInfoFishing(_fisher));
		_fisher.broadcastCharInfo();
	...
	private void doDie(boolean win)
	{
		...
		if(win)
			if(/*На оффе спавнится и в мирной зоне: !_fisher.isInPeaceZone() && */Rnd.chance(5))
			{
				win = false;
			...
			else
			{
				...
				ItemInstance item = ItemFunctions.createItem(_fish.getId());
				item.setCount(Config.RATE_FISH_DROP_COUNT);
				if(_fisher.getWeightPenalty() >= 3 || _fisher.getInventoryLimit() * 0.8 < _fisher.getInventory().getSize())
				{
					// TODO: [Bonux] Проверить на оффе, что происходит за данных условий.
				...
				else
				{
					...
					_fisher.sendPacket(SystemMessagePacket.obtainItems(item));
				}
	...
	public static int getRandomFishLvl(Player player)
	{
		...
		{
			if(e.getAbnormalType() == AbnormalType.fishing_mastery_down)
			{
				skilllvl = e.getSkill().getLevel();
				break;
			}
		}
