	...
	public boolean useVoicedCommand(String command, Player activeChar, String args)
	{
		...
		if(!Config.USE_CLIENT_LANG)
		{
			...
			for(Language lang : Config.AVAILABLE_LANGUAGES)
			{
				...
				button = button.replace("<?lang_name?>", StringsHolder.getInstance().getNotNull(lang, "LangFull"));
		...
		if(Config.AUTO_LOOT_INDIVIDUAL)
		{
			...
			if(Config.AUTO_LOOT_ONLY_ADENA)
			{
				...
				if(activeChar.isAutoLootOnlyAdenaEnabled())
					autolootAdena = autolootAdena.replace("<?value_name_adena?>", new CustomMessage("common.Disable", activeChar).toString());
				else
					autolootAdena = autolootAdena.replace("<?value_name_adena?>", new CustomMessage("common.Enable", activeChar).toString());
			}
			...
			if(activeChar.isAutoLootEnabled())
				autolootBlock = autolootBlock.replace("<?value_name_items?>", new CustomMessage("common.Disable", activeChar).toString());
			else
				autolootBlock = autolootBlock.replace("<?value_name_items?>", new CustomMessage("common.Enable", activeChar).toString());
			...
			if(activeChar.isAutoLootHerbsEnabled())
				autolootBlock = autolootBlock.replace("<?value_name_herbs?>", new CustomMessage("common.Disable", activeChar).toString());
			else
				autolootBlock = autolootBlock.replace("<?value_name_herbs?>", new CustomMessage("common.Enable", activeChar).toString());
		}
