...
public class WhoAmI implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player player, String args)
	{
		...
		double pCritPerc = player.getPCriticalDmg(target, null) * 100.;
		double pCritStatic = player.calcStat(Stats.P_CRITICAL_DAMAGE_STATIC, target, null);
		double mCritPerc = player.getMagicCriticalDmg(target, null) * 100.;
		...
		double pCritSkillRate = player.calcStat(Stats.SKILL_CRIT_CHANCE_MOD, target, null) - 100.;
		double pCritSkillDam = player.calcStat(Stats.SKILL_CRIT_DAM_MOD, target, null) * 100. * 2.;
		...
		double xpRate = player.getRateExp(false,true);
		double spRate = player.getRateSp(false,true);
		...
		double critDamResistStatic = player.calcStat(Stats.P_CRIT_DAMAGE_RECEPTIVE, target, null);
		double critDamResist = 100. - 100 * (player.calcStat(Stats.P_CRIT_DAMAGE_RECEPTIVE, 1., target, null) - critDamResistStatic);
		...
		sb.replaceFirst("%pCritSkillRate%", df.format(pCritSkillRate));
		sb.replaceFirst("%pCritSkillDam%", df.format(pCritSkillDam));
