...
public class BlessedSpiritShotItemHandler extends DefaultItemHandler
{
	...
	static
	{
		SHOT_SKILLS.put(0, SkillHolder.getInstance().getSkill(2061, 1)); // None Grade
		SHOT_SKILLS.put(1, SkillHolder.getInstance().getSkill(2160, 1)); // D Grade
		SHOT_SKILLS.put(2, SkillHolder.getInstance().getSkill(2161, 1)); // C Grade
		SHOT_SKILLS.put(3, SkillHolder.getInstance().getSkill(2162, 1)); // B Grade
		SHOT_SKILLS.put(4, SkillHolder.getInstance().getSkill(2163, 1)); // A Grade
		SHOT_SKILLS.put(5, SkillHolder.getInstance().getSkill(2164, 1)); // S Grade
		SHOT_SKILLS.put(6, SkillHolder.getInstance().getSkill(9195, 1)); // R Grade
	};
	...
	public boolean useItem(Playable playable, ItemInstance item, boolean ctrl)
	{
		...
		if(!player.isFake() && !player.getInventory().destroyItem(item, bspsConsumption))
		{
			if(isAutoSoulShot)
			{
				player.removeAutoSoulShot(shotId);
		...
		Skill skill = player.getAdditionalSSEffect(true, true);
		if(skill == null)
