...
public class AdminSkill implements IAdminCommandHandler
{
	...
	private void adminAddSkill(Player activeChar, String[] wordList)
	{
		...
		if(wordList.length >= 2)
		{
			...
			if(skill != null)
			{
				player.sendMessage("Admin gave you the skill " + skill.getName(player) + ".");
				player.addSkill(skill, true);
				player.sendSkillList();
				activeChar.sendMessage("You gave the skill " + skill.getName(activeChar) + " to " + player.getName() + ".");
			}
			else
	...
	private void adminRemoveSkill(Player activeChar, String[] wordList)
	{
		...
		if(wordList.length == 2)
		{
			...
			if(skill != null)
			{
				player.sendMessage("Admin removed the skill " + skill.getName(player) + ".");
				player.removeSkill(skill, true);
				player.sendSkillList();
				activeChar.sendMessage("You removed the skill " + skill.getName(activeChar) + " from " + player.getName() + ".");
			}
			else
				activeChar.sendMessage("Error: there is no such skill.");
		}
