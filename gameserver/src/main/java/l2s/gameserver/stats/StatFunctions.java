...
public class StatFunctions
{
	...
	private static class FuncPCritDamMul extends Func
	{
		...
		{
			super(Stats.P_CRITICAL_DAMAGE, 0x20, null);
		}

		@Override
		public void calc(Env env)
		{
			//env.value += BaseStats.STR.calcBonus(env.character) - 1;
			env.value *= Math.sqrt(BaseStats.STR.calcBonus(env.character));
	...
	{
		static final FuncMCritDamMul func = new FuncMCritDamMul();

		private FuncMCritDamMul()
		{
			super(Stats.M_CRITICAL_DAMAGE, 0x20, null);
		}

		@Override
		public void calc(Env env)
		{
			//env.value += BaseStats.INT.calcBonus(env.character) - 1;
			//env.value *= BaseStats.INT.calcBonus(env.character);
			env.value *= Math.sqrt(BaseStats.INT.calcBonus(env.character));
		}
	}
	...
	private static class FuncAttackRange extends Func
	{
		...
		public void calc(Env env)
		{
			if(env.character.isPlayer() && env.character.isTransformed())
			{
				WeaponType attackType = env.character.getTransform().getBaseAttackType();
				if(attackType != WeaponType.NONE)
				{
					env.value += WeaponTemplate.getDefaultAttackRange(attackType);
					return;
				}
			}
	...
	private static class FuncPDamageResists extends Func
	{
		...
		public void calc(Env env)
		{
			...
			env.value *= 0.01 * env.target.calcStat(env.character.getBaseStats().getAttackType().getDefence(), env.character, env.skill);
	...
	private static class FuncMaxHpLimit extends Func
	{
		...
		public void calc(Env env)
		{
			env.value = Math.min(Config.HP_MP_CP_LIMIT, env.value);
		}
	}
	...
	private static class FuncCAtkLimit extends Func
	{
		...
		{
			super(Stats.P_CRITICAL_DAMAGE, 0x100, null);
		}
	...
	private static class FuncCMAtkLimit extends Func
	{
		static final Func func = new FuncCMAtkLimit();

		private FuncCMAtkLimit()
		{
			super(Stats.M_CRITICAL_DAMAGE, 0x100, null);
		}

		@Override
		public void calc(Env env)
		{
			env.value = Math.min(Config.LIM_MCRIT_DAM, env.value);
		}
	}
	...
	private static class FuncAttributeAttackInit extends Func
	{
		...
		public void calc(Env env)
		{
			/*int val = env.character.getBaseStats().getAttributeAttack()[element.getId()] == 0 && env.character.isRaid() && env.character.getLevel() >= 80 ? (env.character.getLevel() >= 90 ? 300 : 150) : env.character.getBaseStats().getAttributeAttack()[element.getId()];*/
	...
	private static class FuncAttributeDefenceInit extends Func
	{
		...
		public void calc(Env env)
		{
			int val = env.character.isMonster() && env.character.getBaseStats().getAttributeDefence()[element.getId()] == 0 /*&& env.character.isRaid()*/ && env.character.getLevel() >= 80 ? (env.character.getLevel() >= 90 ? (env.character.isRaid() ? 300 : 250) : (env.character.isRaid() ? 150 : 100)) : env.character.getBaseStats().getAttributeDefence()[element.getId()];
			env.value += val; //env.character.getBaseStats().getAttributeDefence()[element.getId()];
		}
	}
	...
	public static void addPredefinedFuncs(Creature cha)
	{
		...
		cha.addStatFunc(FuncCMAtkLimit.func);
