...
public class DefaultAI extends CharacterAI
{
	...
	public DefaultAI(NpcInstance actor)
	{
		...
		_maxPursueRange = actor.getParameter("MaxPursueRange", actor.isRaid() ? Config.MAX_PURSUE_RANGE_RAID : npc.isUnderground() ? Config.MAX_PURSUE_UNDERGROUND_RANGE : Config.MAX_PURSUE_RANGE);
	...
	protected void thinkAttack()
	{
		...
		if(!actor.isInRange(loc, getMaxPursueRange()) && !(actor instanceof DecoyInstance))
		{
			teleportHome();
			return;
		}
	...
	protected boolean tryMoveToTarget(Creature target, int range)
	{
		...
		if(_pathfindFails >= getMaxPathfindFails() && (System.currentTimeMillis() > getAttackTimeout() - getMaxAttackTimeout() + getTeleportTimeout()) && actor.isInRange(target, getMaxPursueRange()))
		{
			_pathfindFails = 0;
	...
	protected boolean doTask()
	{
		...
		// Задание "прибежать в заданные координаты"
			...
			case ATTACK:
			{
				...
				if(!checkTarget(target, getMaxPursueRange()))
					return true;
			...
			case CAST:
			{
				...
				if(!checkTarget(target, getMaxPursueRange() + castRange))
					return true;
				...
				if(actor.getRealDistance3D(target) <= castRange + 60 && GeoEngine.canSeeTarget(actor, target, false))
				{
					...
					_pathfindFails = 0;
					setAttackTimeout(getMaxAttackTimeout() + System.currentTimeMillis());
					actor.doCast(skill, isAoE ? actor : target, !target.isPlayable());
					return maybeNextTask(currentTask);
				}
	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		if(!attacker.isSymbolInstance())
			actor.getAggroList().addDamageHate(attacker, 0, damage);
		else if(attacker.getPlayer() != null)
			actor.getAggroList().addDamageHate(attacker.getPlayer(), 0, damage); //avoid attacking the symbol itself!		
	...
	protected void onEvtAggression(Creature attacker, int aggro)
	{
		...
		if(!attacker.isSymbolInstance())
			...
			actor.getAggroList().addDamageHate(attacker.getPlayer(), 0, aggro); //avoid attacking the symbol itself!
	...
	protected void checkUD(Creature attacker)
	{
		...
		else
		{
			...
			{
				getActor().getEffectList().stopEffects(UD_SKILL);
				_UDFlag = UD_NONE;
			}
		}
	}
	...
	protected Creature prepareTarget()
	{
		...
		{
			Creature randomHated = actor.getAggroList().getRandomHated(getMaxPursueRange());
			if(randomHated != null)
			{
		...
		if(skill != null)
		{
			...
			if(actor.isMovementDisabled() && distance > skill.getAOECastRange() + 60)
			{
				...
				if(skill.isOffensive())
				{
					...
					for(Creature cha : actor.getAggroList().getHateList(getMaxPursueRange()))
					{
