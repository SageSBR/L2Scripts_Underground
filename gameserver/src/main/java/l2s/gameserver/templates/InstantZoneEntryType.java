...
public enum InstantZoneEntryType
{
	...
	PARTY
	{
		...
		public boolean canEnter(Player player, InstantZone instancedZone)
		{
			...
			if(party.getMemberCount() < instancedZone.getMinParty() && player.getAccessLevel() < 100)
			{
			...
			if(party.getMemberCount() > instancedZone.getMaxParty() && player.getAccessLevel() < 100)
			{
		...
		public boolean canReEnter(Player player, InstantZone instanceZone)
		{
			...
			if(party.getMemberCount() > instanceZone.getMaxParty() && player.getAccessLevel() < 100)
			{
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
				return false;
			}
	...
	COMMAND_CHANNEL
	{
		...
		{
			Party party = player.getParty();
			if(party == null || party.getCommandChannel() == null && player.getAccessLevel() < 100)
			{
			...
			if(cc == null && player.getAccessLevel() >= 100)
			{
				for(Player member : party.getPartyMembers())
				{
					if(!player.isInRange(member, 500))
			...
			{
				if (cc.getMemberCount() < instancedZone.getMinParty() && player.getAccessLevel() < 100) {
					player.sendPacket(new SystemMessagePacket(SystemMsg.YOU_MUST_HAVE_A_MINIMUM_OF_S1_PEOPLE_TO_ENTER_THIS_INSTANT_ZONE).addInteger(instancedZone.getMinParty()));
					return false;
				}
				if (cc.getMemberCount() > instancedZone.getMaxParty() && player.getAccessLevel() < 100) {
					player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
					return false;
				}
		...
		public boolean canReEnter(Player player, InstantZone instanceZone)
		{
			...
			if(commparty == null || commparty.getCommandChannel() == null && player.getAccessLevel() < 100)
			{
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_BECAUSE_YOU_ARE_NOT_ASSOCIATED_WITH_THE_CURRENT_COMMAND_CHANNEL);
				return false;
			}
			...
			if(cc.getMemberCount() > instanceZone.getMaxParty() && player.getAccessLevel() < 100)
			{
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
				return false;
			}
